...
//                registerReceiver(myReceiver, intentFilterConnection);
    ...
    @SuppressLint("MissingPermission")
    private void scandevice () throws IOException {
        ...
        if (network != null) {
            ...
            String gatewayAddress = String.format(
                    ...
                    (ipAddress >> 16 & 0xff),
                    (ipAddress >> 24 & 0xff));
            String prefix = gatewayAddress.substring(0, gatewayAddress.lastIndexOf('.') + 1);
            List<String> connectedDevices = new ArrayList<>();
            for (int i = 1; i < 255; i++) {
                String testIp = prefix + i;
                InetAddress address = InetAddress.getByName(testIp);
                if (address.isReachable(100)) {
                    connectedDevices.add(address.getHostAddress());
                    System.out.println(connectedDevices);
                }

            }
            // List of connected devices is in connectedDevices
        }
    }
